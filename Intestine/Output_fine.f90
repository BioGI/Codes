...
IF(myid .EQ. master) THEN
  ...
  DO k=1,nz_fine
    volume = volume + rDom_fine(k)*rDom_fine(k)*zcf_fine
  END DO

  volume = PI*volume

  WRITE(2461,'(2E15.5)') REAL(iter/(nt/nPers)), volume
  CALL FLUSH(2461)  
...
Drug_Remained_in_Domain = phiDomain * zcf3  !The units of concentration is supposed to be mol/cm^3. Not sure though.

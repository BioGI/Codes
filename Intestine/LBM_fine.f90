       ...
       IF ( flagParticleCF(current%pardata%parid) ) THEN  !Check if particle is in coarse mesh
          ...
          ELSE IF ( (V_eff_Ratio .GT. 1.0) .AND. (V_eff_Ratio .LT. 27.0 ) ) THEN		
             ...
             overlapFineProc = MAX ( MIN(VIB_x(2), iMaxDomain_fine(mySub)*xcf_fine ) - MAX(VIB_x(1), (iMinDomain_fine(mySub)-1)*xcf_fine), 0.0_dbl)  * &
                  ...
                  MAX ( MIN(VIB_z(2),kMaxDomain_fine(mySub)*zcf_fine) - MAX(VIB_z(1),(kMinDomain_fine(mySub)-1)*zcf_fine) , 0.0_dbl)
          ...
          ELSE IF (V_eff_Ratio .GE. 27.0) THEN                             
             ...
             if (overlapFineProc .gt. 0) then
                ...
                NEP_x(2)= MIN(nxSub_fine-1, CEILING(xp + 0.5*L_influence_P/xcf))
                NEP_y(1)= MAX(2,FLOOR(yp - 0.5*L_influence_P/xcf_fine))
                NEP_y(2)= MIN(nySub_fine-1, CEILING(yp + 0.5*L_influence_P/ycf))

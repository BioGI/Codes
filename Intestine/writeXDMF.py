...
def writeFields(fo):
    ...
    for index in range(np.size(timeDirs)):
        ...
        timeName = timeDirs['iterations'][index]


        fo.write('      <Grid Name="mesh1" GridType="Uniform">\n')
        fo.write('        <Time Type="Single" Value="{}" />\n'.format(timeName))
        fo.write('        <Topology TopologyType="3DCoRectMesh" NumberOfElements="160 81 81 "/>\n')
        fo.write('        <Geometry GeometryType="ORIGIN_DXDYDZ">\n')
        fo.write('        <DataItem Dimensions="3" NumberType="Float" Precision="4" Format="XML">\n')
        fo.write('        0 0 0\n')
        fo.write('        </DataItem>\n')
        fo.write('        <DataItem Dimensions="3" NumberType="Float" Precision="4" Format="XML">\n')
        fo.write('        1.5e-4 1.5e-4 1.5e-4\n')
        fo.write('        </DataItem>\n')
        fo.write('        </Geometry>\n')
        
        fo.write('        <Attribute Name="U" AttributeType="Vector" Center="Cell">\n')
        fo.write('        <DataItem ItemType="Function"  Function="JOIN($0,$1,$2)" Dimensions="160 81 81 3">\n')
        fo.write('        <DataItem Dimensions="160 81 81 " NumberType="Float" Precision="4" Format="HDF">\n')
        fo.write('        out-{:07d}.h5:/Ux\n'.format(timeName))
        fo.write('        </DataItem>\n')
        fo.write('        <DataItem Dimensions="160 81 81 " NumberType="Float" Precision="4" Format="HDF">\n')
        fo.write('        out-{:07d}.h5:/Uy\n'.format(timeName))
        fo.write('        </DataItem>\n')
        fo.write('        <DataItem Dimensions="160 81 81 " NumberType="Float" Precision="4" Format="HDF">\n')
        fo.write('        out-{:07d}.h5:/Uz\n'.format(timeName))
        fo.write('        </DataItem>\n')
        fo.write('        </DataItem>\n')
        fo.write('        </Attribute>\n')

        fo.write('        <Attribute Name="node" AttributeType="Scalar" Center="Cell">\n')
        fo.write('        <DataItem Dimensions="160 81 81 " NumberType="Float" Precision="4" Format="HDF">\n')
        fo.write('        out-{:07d}.h5:/node\n'.format(timeName))
        fo.write('        </DataItem>\n')
        fo.write('        </Attribute>\n')

        fo.write('        <Attribute Name="phi" AttributeType="Scalar" Center="Cell">\n')
        fo.write('        <DataItem Dimensions="160 81 81 " NumberType="Float" Precision="4" Format="HDF">\n')
        fo.write('        out-{:07d}.h5:/phi\n'.format(timeName))
        fo.write('        </DataItem>\n')
        fo.write('        </Attribute>\n')

        fo.write('        <Attribute Name="P" AttributeType="Scalar" Center="Cell">\n')
        fo.write('        <DataItem Dimensions="160 81 81 " NumberType="Float" Precision="4" Format="HDF">\n')
        fo.write('        out-{:07d}.h5:/P\n'.format(timeName))
        fo.write('        </DataItem>\n')
        fo.write('        </Attribute>\n')
...
print "Reading time list from 'timeFiles'. HDF5 file names are assumed to be out-timeName.h5"
timeDirs = np.loadtxt('timeFiles', dtype={'names':('iterations',), 'formats' : (np.int,)} )

with open('intestineData.xdmf','w') as fo:
    writeFields(fo)

...
!	ix0=MIN(FLOOR(xp),NxSub)
!	ix1=MIN(FLOOR(xp)+1_lng,NxSub)
!	iy0=MIN(FLOOR(yp),NySub)
!	iy1=MIN(FLOOR(yp)+1_lng,NySub)
!	iz0=MIN(FLOOR(zp),NzSub)
!	iz1=MIN(FLOOR(zp)+1_lng,NzSub)
...
!	ix0=MAX(FLOOR(xp),iMin-1)
!	ix1=MIN(CEILING(xp),iMax+1)
!	iy0=MAX(FLOOR(yp),jMin-1)
!	iy1=MIN(CEILING(yp),jMax+1)
!	iz0=MAX(FLOOR(zp),kMin-1)
!	iz1=MAX(CEILING(zp),kMax+1)
...
!	END IF
	...
	!ix0=MAX(MIN(FLOOR(xp),iMax),iMin)
	!ix1=MAX(MIN(FLOOR(xp),iMax),iMin)!+1_lng
	!iy0=MAX(MIN(FLOOR(yp),jMax),jMin)
	!iy1=MAX(MIN(FLOOR(yp),jMax),jMin)!+1_lng
	!iz0=MAX(MIN(FLOOR(zp),kMax),kMin)
	!iz1=MAX(MIN(FLOOR(zp),kMax),kMin)!+1_lng
	
!	write(9,*) iter,current%pardata%xp,current%pardata%yp,current%pardata%zp
!	write(9,*) iter,x(ix0),y(iy0),z(iz0),x(ix1),y(iy1),z(iz1)
!	write(9,*) iter,u(ix0,iy0,iz0),u(ix1,iy1,iz1)
!	write(9,*) iter,ix0,iy0,iz0,ix1,iy1,iz1,mySub
!	write(9,*) iter,iMin,jMin,kMin,iMax,jMax,kMax,mySub
!	write(9,*) iter,iMinDomain(mySub),jMinDomain(mySub),kMinDomain(mySub),iMaxDomain(mySub),jMaxDomain(mySub),kMaxDomain(mySub),mySub

!	if (iz0.GT.nzSub) then
!		iz0 = iz0-1_lng
!	elseif (iz0.LT.1) then
!		iz0 = iz0 +1_lng
!	endif
...
!	write(*,*) 'In Interp_parvel, before zd calc',iz0,iz1,kMax,kMin,current%pardata%zp,mySub,iter
...
!	write(*,*) 'In Interp_parvel, after zd calc',iz0,iz1,kMax,kMin,current%pardata%zp,mySub,iter
...
!	write(*,*) 'In Interp_parvel, after uinterp',iz0,iz1,kMax,kMin,current%pardata%zp,mySub,iter

!	write(*,*) 'test',w(ix0,iy0,iz0)*vcf,w(ix1,iy1,iz1)*vcf,node(ix0,iy0,iz0),node(ix1,iy1,iz1),c,xd,yd,zd,ix0,current%pardata%xp,x(ix0)/xcf
!	write(*,*) 'test',w(ix0,iy0,iz0),w(ix1,iy1,iz1),node(ix0,iy0,iz0),node(ix1,iy1,iz1),c,xd,yd,zd,ix0,current%pardata%xp,x(ix0)/xcf
!	write(*,*) 'test',node(ix0,iy0,iz0),ix0,iy0,iz0,mySub,current%pardata%xp,current%pardata%yp,current%pardata%zp,nxSub,nySub,nzSub
...
DO k=1,nzSub
  ...
  END DO
END DO
!Cb_global = Cb_global/numFluids
!Cb_numFluids = numFluids
!write(*,*) Cb_global,mySub,Cb_numFluids
!------------------------------------------------
...
DO WHILE (ASSOCIATED(current))
	...
	!current%pardata%bulk_conc=Cb_global!0.0_dbl
	!temp = current%pardata%rpold**2.0_dbl-4.0_dbl*tcf*molarvol*diffm*current%pardata%sh*max((current%pardata%par_conc-current%pardata%bulk_conc),0.0_dbl)
	...
	ELSE
		...
		current%pardata%rp=0.5_dbl*(current%pardata%rpold+sqrt(temp))
            	...
            	!WRITE(1000,'(A75)') "error in LBM.f90 at Line 759: Calc_Scalar_Release"
            	!WRITE(1000,*) "b^2-4ac < 0"
            	!CLOSE(1000)
            	!!STOP
	END IF
	deltaR=current%pardata%rpold-current%pardata%rp
	!!bulkVolume=xcf*ycf*zcf
	!current%pardata%delNBbyCV=4.0_dbl*PI*current%pardata%rp*current%pardata%rp*deltaR*current%pardata%par_conc/(bulkVolume*(1.0_dbl-current%pardata%gamma_cont))
	
	!bulkVolume=1.0
	!current%pardata%rpold=current%pardata%rpold/0.005_dbl
	!current%pardata%rp=current%pardata%rp/0.005_dbl
	...
	!current%pardata%delNBbyCV = 1.0e-6
	!*current%pardata%par_conc/(bulkVolume*(1.0_dbl-current%pardata%gamma_cont))
	!/(molarvol*bulkVolume*(1.0_dbl-current%pardata%gamma_cont))

	!current%pardata%delNBbyCV = 0.0_dbl
	!write(*,*) current%pardata%parid,current%pardata%delNBbyCV
...
!	! shear-interpolation to aprticle location
!	! Do first level linear interpolation in x-direction
!	c00 = phi(ix0,iy0,iz0)*(1.0_dbl-xd)+phi(ix1,iy0,iz0)*xd	
!	c01 = phi(ix0,iy0,iz1)*(1.0_dbl-xd)+phi(ix1,iy0,iz1)*xd	
!	c10 = phi(ix0,iy1,iz0)*(1.0_dbl-xd)+phi(ix1,iy1,iz0)*xd	
...
!        current%pardata%bulk_conc=c
	...
	!IF(kb.GT.nz) kb=kb-nz
	!IF(kb.LT.1) kb=kb+nz
	!IF(jb.GT.ny) jb=jb-ny
	!IF(jb.LT.1) jb=jb+ny
...
DO WHILE (ASSOCIATED(current))
	...
	!delphi_particle(ix0,iy0,iz0)=delphi_particle(ix0,iy0,iz0)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix0,iy0,iz0)/bulk_conc(i))
	!delphi_particle(ix0,iy1,iz0)=delphi_particle(ix0,iy1,iz0)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix0,iy1,iz0)/bulk_conc(i))
	!delphi_particle(ix1,iy0,iz0)=delphi_particle(ix1,iy0,iz0)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix1,iy0,iz0)/bulk_conc(i))
	!delphi_particle(ix1,iy1,iz0)=delphi_particle(ix1,iy1,iz0)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix1,iy1,iz0)/bulk_conc(i))
	!delphi_particle(ix0,iy0,iz1)=delphi_particle(ix0,iy0,iz1)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix0,iy0,iz1)/bulk_conc(i))
	!delphi_particle(ix0,iy1,iz1)=delphi_particle(ix0,iy1,iz1)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix0,iy1,iz1)/bulk_conc(i))
	!delphi_particle(ix1,iy0,iz1)=delphi_particle(ix1,iy0,iz1)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix1,iy0,iz1)/bulk_conc(i))
	!delphi_particle(ix1,iy1,iz1)=delphi_particle(ix1,iy1,iz1)+current%pardata%delNBbyCV*0.125_dbl!(phi(ix1,iy1,iz1)/bulk_conc(i))
	...
	!delphi_particle(ix0,iy0,iz0)=delphi_particle(ix0,iy0,iz0)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix0,iy0,iz0)/bulk_conc(i))
	!delphi_particle(ix0,iy1,iz0)=delphi_particle(ix0,iy1,iz0)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix0,iy1,iz0)/bulk_conc(i))
	!delphi_particle(ix1,iy0,iz0)=delphi_particle(ix1,iy0,iz0)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix1,iy0,iz0)/bulk_conc(i))
	!delphi_particle(ix1,iy1,iz0)=delphi_particle(ix1,iy1,iz0)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix1,iy1,iz0)/bulk_conc(i))
	!delphi_particle(ix0,iy0,iz1)=delphi_particle(ix0,iy0,iz1)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix0,iy0,iz1)/bulk_conc(i))
	!delphi_particle(ix0,iy1,iz1)=delphi_particle(ix0,iy1,iz1)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix0,iy1,iz1)/bulk_conc(i))
	!delphi_particle(ix1,iy0,iz1)=delphi_particle(ix1,iy0,iz1)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix1,iy0,iz1)/bulk_conc(i))
	!delphi_particle(ix1,iy1,iz1)=delphi_particle(ix1,iy1,iz1)+current%pardata%delNBbyCV*1.0_dbl!(phi(ix1,iy1,iz1)/bulk_conc(i))
	...
	c111 = MAX(MIN(ax1,bx1)-MAX(ax0,bx0),0.0_dbl)*MAX(MIN(ay1,by1)-MAX(ay0,by0),0.0_dbl)*MAX(MIN(az1,bz1)-MAX(az0,bz0),0.0_dbl)
        ...
        !if (csum.eq.0.0) then
        !        write(*,*) 'csum is zero',delta_par,current%pardata%parid,ax0,ay0,az0,ax1,ay1,az1,ix0,iy0,iz0,ix1,iy1,iz1
        !endif

        !if (mySub.eq.22) then
        !write(*,*) current%pardata%parid,delta_par,current%pardata%delNBbyCV,c000,c010,c100,c110,c001,c011,c101,c111
        !end if
	...
	!write(*,*) ix0,iy0,iz0,delphi_particle(ix0,iy0,iz0),current%pardata%delNBbyCV

	!IF (zp.GT.REAL(kMax,dbl)) THEN
	!delphi_particle(ix0,iy0,iz0)=delphi_particle(ix0,iy0,iz0)+current%pardata%delNBbyCV!(phi(ix0,iy0,iz0)/bulk_conc(i))
	!ELSE IF (zp.LT.REAL(kMin,dbl)) THEN
	!delphi_particle(ix0,iy0,iz1)=delphi_particle(ix0,iy0,iz1)+current%pardata%delNBbyCV!(phi(ix0,iy0,iz0)/bulk_conc(i))
	!ELSE
	!delphi_particle(ix0,iy0,iz0)=delphi_particle(ix0,iy0,iz0)+current%pardata%delNBbyCV!(phi(ix0,iy0,iz0)/bulk_conc(i))
	!END IF
...
!		current%pardata%zp = MOD(current%pardata%zp,REAL(nz,dbl))
!	ENDIF
!	write(*,*) 'before ',mySub,current%pardata%new_part,current%pardata%cur_part,ParticleTransfer
!	! Estimate to which partition the updated position belongs to.
!	DO ipartition = 1_lng,NumSubsTotal 
!		IF ((current%pardata%xp.GE.REAL(iMinDomain(ipartition),dbl)-0.5_dbl).AND.&
!		(current%pardata%xp.LT.(REAL(iMaxDomain(ipartition),dbl)+0.5_dbl)).AND. &
!		(current%pardata%yp.GE.REAL(jMinDomain(ipartition),dbl)-0.5_dbl).AND. &
!		(current%pardata%yp.LT.(REAL(jMaxDomain(ipartition),dbl)+0.5_dbl)).AND. &
!		(current%pardata%zp.GE.REAL(kMinDomain(ipartition),dbl)-0.5_dbl).AND. &
!		(current%pardata%zp.LT.(REAL(kMaxDomain(ipartition),dbl)+0.5_dbl))) THEN
!			current%pardata%new_part = ipartition
!		END IF
!	END DO
!	
!	write(*,*) 'after1 ',mySub,current%pardata%new_part,current%pardata%cur_part,ParticleTransfer
!	IF ((.NOT.ParticleTransfer).AND.(current%pardata%new_part /= current%pardata%cur_part)) THEN
!		ParticleTransfer = .TRUE.
!	END IF
!	
!	write(*,*) 'after2 ',mySub,current%pardata%new_part,current%pardata%cur_part,ParticleTransfer
...
!write(*,*) 'kmin and kmax in partition ',mySub,' are ',kMin,kMax,' with range from 0 to ',nzSub,' nxSub and nySub are ',nxSub,nySub
!write(*,*) 'imin and imax in partition ',mySub,' are ',iMin,iMax,' with range from 0 to ',nxSub,' nzSub and nySub are ',nzSub,nySub
!write(*,*) 'jmin and jmax in partition ',mySub,' are ',jMin,jMax,' with range from 0 to ',nySub,' nxSub and nzSub are ',nxSub,nzSub
...
!	write(*,*) 'before ',mySub,current%pardata%new_part,current%pardata%cur_part,current%pardata%zp,ParticleTransfer,(kMinDomain(i), i=1,4),(kMaxDomain(i), i=1,4)
	! Estimate to which partition the updated position belongs to.
	!ParticleTransfer = .FALSE.
	DO ipartition = 1_lng,NumSubsTotal 
...
!	write(*,*) iter,'after1 ',mySub,current%pardata%new_part,current%pardata%cur_part,current%pardata%zp,ParticleTransfer

	IF ((.NOT.ParticleTransfer).AND.(current%pardata%new_part .NE. current%pardata%cur_part)) THEN
		ParticleTransfer = .TRUE.
	END IF
	
!	write(*,*) iter,'after2 ',current%pardata%parid,mySub,current%pardata%cur_part,current%pardata%new_part,current%pardata%zp,ParticleTransfer
...
!    DO i=0,nxSub+1
	...
	!write(9,*) "In macro before",rho(i,j,k),i,j,k,node(i,j,k)
        DO m=0,NumDistDirs  
...
!	! Begin Added to fix mass conservation
!	nodebounflag = .FALSE.
!        DO m=0,NumDistDirs 
!		ii = i+ex(m) 
!		jj = j+ex(m) 
!		kk = k+ex(m)
!		IF (node(ii,jj,kk) .NE. FLUID) THEN
!			nodebounflag = .TRUE.
!		END IF
!        END DO
!	IF (nodebounflag) THEN
!		rho(i,j,k) = 1.0_dbl
!	END IF
!	! End Added to fix mass conservation
